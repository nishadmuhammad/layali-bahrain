...
@section('formBody')
    ...
    <div class="col-6">
        <div class="form-group">
            ...
            <input type="text" id="email" name="email" required class="form-control {{$errors->has('email') ? 'is-invalid' : ''}}" value="{{$user->email}}" >
            @if($errors->has('email'))
                <span class="help-block error invalid-feedback">
                ...
                @foreach($roles as $role)
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="{{$role->name}}" name="role[]"
                                   value="{{$role->name}}" {{ (is_array($userRole) and in_array($role->name, $userRole)) ? ' checked' : '' }}>
